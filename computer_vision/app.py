...
def menu():
    ...
    with st.sidebar:
        ...
        selected = option_menu("Menu", ["Metrics"], icons=["plus"], menu_icon="three-dots",default_index=0)
...
def iou_plot():
    ...
    with st.expander(""):
        ...
        if image != None:
            ...
            canvas = create_canvas(ID, image, label_color)
...
def create_canvas(ID, image, label_color):
    
    fill_color = "rgb(255, 165, 0, 0.3)"
        
    canvas = st_canvas(fill_color=label_color, width=660, height=440, stroke_width=2, stroke_color="#eee", background_image=Image.open(image).resize((660, 420)), drawing_mode="rect", key=ID)
...
if __name__ == "__main__":
    check_states()
    menu()
    if st.session_state.sidebar_selected == "Metrics":
        metrics()
